...
1. **反应物输入识别**：支持标准化学式（`Fe`、`CuSO4·5H2O`）与物质中文名称（`铁`、`胆矾`、`熟石灰` 等 200+ 词条），多种物质用 `+` 分隔；内置公式键盘完全绕开系统输入法联想，且**任意大小写混输均可识别**（`kmno4` / `KMNO4` / `KMno4` → `KMnO4`）；无法识别时明确提示。
2. **全自动生成物推导**：内置经典反应库 + 规则推理，覆盖初高中考试高频反应：燃烧、还原（H₂/C/CO/铝热）、金属+酸/盐/水、金属活动性顺序、中和、复分解（溶解性表判沉淀）、化合、分解、卤素置换、铵根检验、铝的两性、铁三角、铜+浓硫酸/稀硝酸、氮硫氧化链（NO→NO₂→HNO₃、合成氨、氨催化氧化）、Na₂O₂ 系列、氯系列（氯水/84消毒液/制漂白粉）等；无法发生的组合会给出原因（如「铜活动性弱于氢」），绝不乱配。
