...
- 当前版本：`V26.8.41.CALC`（versionCode 2608410）
...
7. **设置中心**：主页右上角进入；结果小数位 1/2/3 位可调（`@system.storage` 持久化，结果页/质量页即时生效）。
8. **常量显示**：阿伏伽德罗常数、气体摩尔体积等常用化学常数速查。

## 页面结构

| 路由 | 说明 |
|------|------|
| `pages/index` | 主页：功能菜单（反应计算/元素查询/质量计算/常用物质表）+ 右上角设置入口 |
...
| `pages/settings` | 设置中心：结果小数位、常量显示、关于 |
