...
- 当前版本：`V26.9.49.CALC`（versionCode 2609490）
...
5. **离线元素数据库**：118 元素全量 standard 相对原子质量 + 原子序数 + 中文名（人教版附录周期表全集）+ 金属活动性顺序 + 常见离子化合价 + 溶解性表；反应引擎完整覆盖人教版九年级上下册全部化学方程式。
6. **一键重置**：清空按钮随时重新输入，无限次计算。
7. **合成查询（产物逆推）**：输入产物（`水` / `H2O`）列出全部能合成它的反应；再补一种已知反应物（`水 氢气`），即可逆推出还缺的反应物（如 `O₂`），点击路径可直接查看配平方程式。
8. **设置中心**：主页右上角进入；结果小数位 1/2/3 位可调、常亮显示开关（应用内保持屏幕常亮，退出重开自动恢复记忆）、按键振动开关（`@system.storage` 持久化）。
9. **常量显示**：阿伏伽德罗常数、气体摩尔体积、水密度、空气组成、爆炸极限等常用中学化学常数速查。
10. **化学速查**：元素周期表（118 全量按原子序数）/ 金属活动性顺序 / 常见元素化合价（含化合价口诀）/ 常见根与酸根化合价（含根价口诀）/ 常见离子符号（36 种全 ASCII 展示：化学式 + 副行"电荷 3+（读作 三加）"，兼容手表字体）/ 常见不溶物 / 常见物质颜色 / 常见气体制取与检验 / 相对原子质量表九组速查，滚动分组展示。
11. **化学专属输入法**：中文连打词库替换为化学专属（物质名 + 118 元素名 + 仪器/概念术语，整词候选优先）；英文模式同按中文逻辑联想——输入字母先聚入候选行，列表第一个为输入本身、随后为元素符号前缀补全（如 c → c, ca, cd, ce, cl, co…），点选上屏。

## 页面结构

| 路由 | 说明 |
|------|------|
| `pages/index` | 主页：功能菜单（反应计算/元素查询/合成查询/质量计算/常用物质表/化学速查）+ 右上角设置入口 |
| `pages/reaction` | 反应计算页：考点阅读器同款输入法（中文/日文/英文）输入反应物 |
| `pages/element` | 元素查询页：按元素检索本地反应库（252 条方程式索引） |
| `pages/synth` | 合成查询页：按产物逆推合成路径与缺失反应物 |
| `pages/result` | 方程式页：带下标渲染的配平方程式、反应类型、条件、各物质相对分子质量（卡片高度随方程式折行自适应） |
| `pages/mass` | 质量计算页：选择已知物质 → 数字键盘输入质量 → 批量推算结果 |
| `pages/catalog` | 常用物质表：分组浏览（顶部渐变遮罩过渡），点选自动填入 |
| `pages/reference` | 化学速查：活动性/化合价/酸根/沉淀/相对原子质量分组速查 |
| `pages/settings` | 设置中心：结果小数位、常亮显示、按键振动、常量显示、关于 |
| `pages/constants` | 常量显示：常用化学常数只读列表 |
| `pages/about` | 关于：版本、包名、数据说明 |
...
│   │       ├── parser.js       # 化学式解析（带结果缓存）
│   │       ├── balance.js      # 配平算法
│   │       ├── reactions.js    # 反应规则引擎（规则 + 50+ 特殊反应库）
│   │       ├── stoich.js       # 计量计算
│   │       ├── constants.js    # 常量显示数据（纯数据）
│   │       └── settings.js     # 设置持久化与输出精度（仅页面层可 import）
│   ├── components/FormulaText/  # 下标渲染组件
│   └── pages/{index,reaction,element,result,mass,catalog,settings,constants,about}/
├── tests/smoke.mjs         # Node 冒烟测试（122 项断言，含九年级考纲全覆盖组）
├── VELA_UI_SKILL.md        # UI 风格开发规范（给后续 AI 的完整指南）
